...
# Версия: 3.3 (Финальная отладка setup.conf в Шаге 99)
# Дата: 2025-04-16
...
step7_start_service_and_extras() {
    ...
    # Отладочные проверки удалены из этой версии
...
step_uninstall() {
    ...
    # Удаляем только cron и скрипты, созданные этим установщиком (скрипт бэкапа)
