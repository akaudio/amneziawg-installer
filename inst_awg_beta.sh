...
initialize_setup() {
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        DISABLE_IPV6=${DISABLE_IPV6:-1}; # По умолчанию 1, если в файле нет
        ALLOWED_IPS_MODE=${ALLOWED_IPS_MODE:-2}; # По умолчанию 2, если в файле нет
        # ALLOWED_IPS загрузится из файла или останется пустым
    ...
    if [[ "$CLI_DISABLE_IPV6" != "default" ]]; then DISABLE_IPV6=$CLI_DISABLE_IPV6; log "Используется значение DISABLE_IPV6 из CLI."; fi
    if [[ "$CLI_ROUTING_MODE" != "default" ]]; then
        ALLOWED_IPS_MODE=$CLI_ROUTING_MODE;
        if [[ "$CLI_ROUTING_MODE" -eq 3 ]]; then ALLOWED_IPS=$CLI_CUSTOM_ROUTES; if [ -z "$ALLOWED_IPS" ]; then die "Не указаны сети для --route-custom."; fi; fi
        log "Используется значение ALLOWED_IPS_MODE из CLI.";
    fi
    ...
    if [[ "$config_exists" -eq 0 ]]; then
         ...
         # Запрашиваем остальные настройки, если они не были заданы через CLI на этом первом запуске
         ...
         # Убедимся, что переменные имеют значения (на случай старого конфига)
         if [[ "$DISABLE_IPV6" == "default" ]]; then DISABLE_IPV6=1; fi
         if [[ "$ALLOWED_IPS_MODE" == "default" ]]; then ALLOWED_IPS_MODE=2; fi
         # Пересчитаем ALLOWED_IPS на основе режима, если он пуст
    ...
    export AWG_PORT AWG_TUNNEL_SUBNET DISABLE_IPV6 ALLOWED_IPS_MODE ALLOWED_IPS; # Экспорт для текущего скрипта
...
# ШАГ 7: Запуск сервиса и доп. настройки (Упрощенная версия)
step7_start_service_and_extras() {
    ...
    # log "Настройка дополнительных компонентов..."; # Убрано
    # setup_auto_updates; setup_backups; setup_log_rotation; # Убрано
...
step_uninstall() {
    ...
    # Удаляем только пакеты AmneziaWG, оставляем unattended-upgrades, если он был установлен
    ...
    log "Удаление файлов..."; rm -rf /etc/amnezia "$AWG_DIR" /etc/modules-load.d/amneziawg.conf /etc/sysctl.d/10-amneziawg-forward.conf /etc/sysctl.d/99-amneziawg-security.conf /etc/logrotate.d/amneziawg* || log_warn "Ошибка удаления файлов."; # Удалены auto-updates и fail2ban из списка
...
# Дополнительные функции безопасности и автоматизации (определения остаются, но не вызываются в Шаге 7)
...
EOF
    chmod +x "$bs" || die "Ошибка chmod $bs"; echo "0 3 * * * root $bs" > /etc/cron.d/backup_amneziawg || log_warn "Ошибка cron"; log "Ежедневный бэкап в $bd настроен."; log "Создание первого бэкапа..."; if "$bs"; then log "OK."; else log_warn "Ошибка первого бэкапа."; fi
}
setup_log_rotation() {
    log "Настройка ротации логов..."; cat > /etc/logrotate.d/amneziawg-installer << EOF || log_warn "Ошибка записи logrotate";
$LOG_FILE
...
create_diagnostic_report() {
    ...
    chmod 600 "$rf" || log_warn "Ошибка chmod отчета."; log "Отчет: $rf";
}
...
while (( current_step < 99 )); do
    ...
    case $current_step in
        ...
        7) step7_start_service_and_extras; current_step=99 ;; # Шаг 7 теперь упрощен
