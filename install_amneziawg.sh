...
initialize_setup() {
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        # Загружаем переменные, присваивая дефолты если переменная пуста
    ...
    # Запрашиваем только если конфига НЕ БЫЛО
    if [[ "$config_exists" -eq 0 ]]; then
         ...
         # Если конфиг был, но что-то не задано, устанавливаем дефолты
         if [[ "$DISABLE_IPV6" == "default" ]]; then DISABLE_IPV6=1; fi
         if [[ "$ALLOWED_IPS_MODE" == "default" ]]; then configure_routing_mode; fi
         log "Используются настройки из $CONFIG_FILE или CLI.";
    fi
...
step_uninstall() {
    ...
    # Запрос подтверждения теперь обязателен и не зависит от интерактивности для безопасности
...
create_diagnostic_report() {
    ...
    { echo "=== AMNEZIAWG DIAGNOSTIC REPORT ==="; date; hostname; echo "--- OS ---"; lsb_release -ds; uname -a; echo ""; echo "--- Configuration ($CONFIG_FILE) ---"; cat "$CONFIG_FILE" 2>/dev/null || echo "N/A"; echo "";
...
while (( current_step < 99 )); do
    ...
    case $current_step in
        ...
        7) step7_start_service_and_extras; current_step=99 ;;
