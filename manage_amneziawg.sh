...
set -o pipefail
AWG_DIR="/root/awg"; SERVER_CONF_FILE="/etc/amnezia/amneziawg/awg0.conf"; SETUP_CONFIG_FILE="$AWG_DIR/setup.conf";
PYTHON_VENV_PATH="$AWG_DIR/venv/bin/python"; AWGCFG_SCRIPT_PATH="$AWG_DIR/awgcfg.py"; LOG_FILE="$AWG_DIR/manage_amneziawg.log";
NO_COLOR=0; # Флаг для отключения цвета

# --- Обработка аргументов ---
VERBOSE_LIST=0; COMMAND=""; ARGS=();
...
SETUP_CONFIG_FILE="$AWG_DIR/setup.conf"; PYTHON_VENV_PATH="$AWG_DIR/venv/bin/python"; AWGCFG_SCRIPT_PATH="$AWG_DIR/awgcfg.py"; LOG_FILE="$AWG_DIR/manage_amneziawg.log";
...
usage() {
    ...
    echo "Опции:"; echo "  -h, --help            Показать эту справку"; echo "  -v, --verbose         Расширенный вывод (для команды list)"; echo "  --no-color            Отключить цветной вывод"; echo "  --conf-dir=ПУТЬ       Указать директорию AWG (по умолч: $AWG_DIR)"; echo "  --server-conf=ПУТЬ    Указать файл конфига сервера (по умолч: $SERVER_CONF_FILE)"; echo "";
