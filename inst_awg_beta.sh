...
# Автор: @bivlked
# Версия: 3.5 (Rename setup.conf + Workaround)
# Дата: 2025-04-18
...
set -o pipefail
AWG_DIR="/root/awg"
CONFIG_FILE="$AWG_DIR/awgsetup_cfg.init" # НОВОЕ ИМЯ ФАЙЛА
...
MANAGE_SCRIPT_PATH="$AWG_DIR/mng_beta.sh" # Имя файла для скрипта управления
...
initialize_setup() {
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        # Устанавливаем дефолты только если переменная пуста после source
        ...
        if [[ -z "$ALLOWED_IPS" ]]; then configure_routing_mode; else export ALLOWED_IPS; fi
    ...
    if [[ "$config_exists" -eq 0 ]]; then
         ...
         if [[ "$DISABLE_IPV6" == "default" ]]; then configure_ipv6; fi # Запросим, если не задан CLI
         if [[ "$ALLOWED_IPS_MODE" == "default" ]]; then configure_routing_mode; fi # Запросим, если не задан CLI
    else
         log "Используются настройки из $CONFIG_FILE (переопределенные CLI, если были). Запрос не требуется.";
         # Убедимся, что переменные установлены (на случай старого конфига)
         ...
         if [[ -z "$ALLOWED_IPS" || "$CLI_ROUTING_MODE" != "default" ]]; then configure_routing_mode; fi
    fi
...
step6_generate_configs() {
    ...
    local temp_conf_path="/root/awgsetup_cfg.init.tmp" # Используем новое имя
    ...
    if [ -f "$temp_conf_path" ]; then
        ...
        # Если временного файла нет, но и основного нет - это проблема
        if [ ! -f "$CONFIG_FILE" ]; then
             log_error "Файл $CONFIG_FILE отсутствует и не может быть восстановлен из временного файла!";
             # НЕ ПРЕРЫВАЕМ УСТАНОВКУ, но предупреждаем. Управление может не работать.
        else
            log "Файл $CONFIG_FILE уже на месте (восстановление не потребовалось)."
        fi
    fi
...
step99_finish() {
    ...
    rm -f /root/awgsetup_cfg.init.tmp
...
step_uninstall() {
    ...
    if [[ -z "$backup" || "$backup" =~ ^[Yy]$ ]]; then
         ...
         # Включаем awgsetup_cfg.init в бэкап
    ...
    log "Удаление файлов..."; rm -rf /etc/amnezia "$AWG_DIR" /etc/modules-load.d/amneziawg.conf /etc/sysctl.d/10-amneziawg-forward.conf /etc/sysctl.d/99-amneziawg-security.conf /etc/logrotate.d/amneziawg* /root/awgsetup_cfg.init.tmp || log_warn "Ошибка удаления файлов.";
