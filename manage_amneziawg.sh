...
# Сначала парсим опции
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help) COMMAND="help"; break ;; # Help - это команда
        ...
        *) # Первый не-опциональный аргумент - команда
             if [ -z "$COMMAND" ]; then COMMAND=$1; else ARGS+=("$1"); fi; shift ;;
    esac
done
# Оставшиеся аргументы (после команды)
ARGS+=("$@")
...
# Назначаем аргументы позиционно (простой вариант для существующих команд)
CLIENT_NAME="${ARGS[0]}"
PARAM="${ARGS[1]}"
VALUE="${ARGS[2]}"
...
check_dependencies() { log "Проверка зависимостей..."; local ok=1; if [ ! -f "$SETUP_CONFIG_FILE" ]; then log_error " - Отсутствует файл настроек: $SETUP_CONFIG_FILE"; ok=0; fi; if [ ! -d "$AWG_DIR/venv" ]; then log_error " - Отсутствует директория venv: $AWG_DIR/venv"; ok=0; fi; if [ ! -f "$AWGCFG_SCRIPT_PATH" ]; then log_error " - Отсутствует скрипт генерации: $AWGCFG_SCRIPT_PATH"; ok=0; fi; if [ ! -f "$SERVER_CONF_FILE" ]; then log_error " - Отсутствует конфиг сервера: $SERVER_CONF_FILE"; ok=0; fi; if [ "$ok" -eq 0 ]; then die "Не найдены необходимые файлы установки AmneziaWG. Убедитесь, что 'install_amneziawg.sh' был успешно завершен."; fi; if ! command -v awg &>/dev/null; then die "Команда 'awg' не найдена."; fi; if [ ! -x "$AWGCFG_SCRIPT_PATH" ]; then die "$AWGCFG_SCRIPT_PATH не найден/не исполняемый."; fi; if [ ! -x "$PYTHON_VENV_PATH" ]; then die "$PYTHON_VENV_PATH не найден/не исполняемый."; fi; log "Зависимости в порядке."; }
...
list_clients() {
    ...
    local verbose=0;
    # Проверяем флаг verbose (первый аргумент после list)
    ...
    done # Не логируем таблицу по умолчанию, только статус в конце
    echo ""; log "Всего клиентов: $tot, Активных/Недавно: $act";
}
usage() {
    exec >&2; echo ""; echo "Скрипт управления AmneziaWG (v2.0)"; echo "=============================================="; echo "Использование: $0 [ОПЦИИ] <КОМАНДА> [АРГУМЕНТЫ]"; echo "";
    echo "Опции:"; echo "  -h, --help            Показать эту справку"; echo "  -v, --verbose         Расширенный вывод (для команды list)"; echo "  --conf-dir=ПУТЬ       Указать директорию AWG (по умолч: $AWG_DIR)"; echo "  --server-conf=ПУТЬ    Указать файл конфига сервера (по умолч: $SERVER_CONF_FILE)"; echo "";
