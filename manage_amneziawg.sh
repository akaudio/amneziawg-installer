...
initialize_setup() {
    ...
    # Не используем tee exec для совместимости и надежности
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        AWG_PORT=${AWG_PORT:-$default_port}; AWG_TUNNEL_SUBNET=${AWG_TUNNEL_SUBNET:-$default_subnet}; DISABLE_IPV6=${DISABLE_IPV6:-"default"}; ALLOWED_IPS_MODE=${ALLOWED_IPS_MODE:-"default"}; ALLOWED_IPS=${ALLOWED_IPS:-""};
        log "Настройки из файла загружены.";
    else
        log "Файл конфигурации $CONFIG_FILE не найден."; AWG_PORT=$default_port; AWG_TUNNEL_SUBNET=$default_subnet; DISABLE_IPV6="default"; ALLOWED_IPS_MODE="default"; ALLOWED_IPS="";
    fi
...
step4_setup_firewall() {
    ...
    setup_improved_firewall || die "Ошибка настройки UFW."; # Используем улучшенную функцию
...
step7_start_service_and_extras() {
    ...
    # Убираем Fail2Ban из автоматической установки
    # setup_fail2ban;
...
step_uninstall() {
    ...
    if ! confirm_action "полностью удалить AmneziaWG и все его данные?" "DEINSTALL"; then exit 1; fi
    ...
    if [[ -z "$backup" || "$backup" =~ ^[Yy]$ ]]; then
         ...
         tar -czf "$bf" -C / etc/amnezia "$AWG_DIR" --ignore-failed-read 2>/dev/null || log_warn "Ошибка бэкапа $bf"; chmod 600 "$bf"; log "Бэкап создан: $bf";
    fi
    ...
    log "Удаление пакетов..."; DEBIAN_FRONTEND=noninteractive apt-get purge -y amneziawg-dkms amneziawg-tools unattended-upgrades &>/dev/null || log_warn "Ошибка purge."; DEBIAN_FRONTEND=noninteractive apt-get autoremove -y &>/dev/null || log_warn "Ошибка autoremove."; # Удалил fail2ban отсюда
    log "Удаление файлов..."; rm -rf /etc/amnezia "$AWG_DIR" /etc/modules-load.d/amneziawg.conf /etc/sysctl.d/10-amneziawg-forward.conf /etc/sysctl.d/99-amneziawg-security.conf /etc/logrotate.d/amneziawg* /etc/apt/apt.conf.d/20auto-upgrades || log_warn "Ошибка rm.";
...
setup_improved_firewall() {
    ...
    if ufw status | grep -q inactive; then
        ...
        ufw limit 22/tcp comment "SSH Rate Limit"; ufw allow "${AWG_PORT}/udp" comment "AmneziaWG VPN"; log "Правила UFW добавлены.";
        log_warn "--- ВКЛЮЧЕНИЕ UFW ---"; log_warn "Проверьте SSH доступ!"; sleep 5;
        read -p "Включить UFW? [y/N]: " confirm_ufw < /dev/tty; if ! [[ "$confirm_ufw" =~ ^[Yy]$ ]]; then log_warn "UFW не включен."; return 1; fi;
...
secure_files() {
    ...
    if [ -f "$CONFIG_FILE" ]; then chmod 600 "$CONFIG_FILE"; fi; if [ -f "$LOG_FILE" ]; then chmod 640 "$LOG_FILE"; fi; if [ -f "$MANAGE_SCRIPT_PATH" ]; then chmod 700 "$MANAGE_SCRIPT_PATH"; fi; log "Права доступа установлены.";
}
setup_auto_updates() {
    ...
    if DEBIAN_FRONTEND=noninteractive dpkg-reconfigure -plow unattended-upgrades; then cat > /etc/apt/apt.conf.d/20auto-upgrades << EOF || { log_warn "Ошибка 20auto-upgrades"; return 1; }; APT::Periodic::Update-Package-Lists "1";\nAPT::Periodic::Unattended-Upgrade "1";\nAPT::Periodic::AutocleanInterval "7";\nAPT::Periodic::Download-Upgradeable-Packages "1";\nUnattended-Upgrade::Remove-Unused-Kernel-Packages "true";\nUnattended-Upgrade::Remove-Unused-Dependencies "true";\nEOF; log "Авто-обновления настроены."; else log_warn "Ошибка dpkg-reconfigure unattended-upgrades."; fi
}
setup_backups() {
...
#!/bin/bash
AWG_DIR="/root/awg"; BACKUP_DIR="$AWG_DIR/backups"; SERVER_CONF_DIR="/etc/amnezia/amneziawg"; TIMESTAMP=$(date +%Y%m%d-%H%M%S); BACKUP_FILE="$BACKUP_DIR/awg_backup_${TIMESTAMP}.tar.gz"; LOG_FILE="$BACKUP_DIR/backup.log"; MAX_BACKUPS=10;
echo "[$TIMESTAMP] Starting backup..." >> "$LOG_FILE"; TEMP_DIR=$(mktemp -d); if [ $? -ne 0 ]; then echo "[$TIMESTAMP] ERROR: Failed to create temp dir." >> "$LOG_FILE"; exit 1; fi; mkdir -p "$TEMP_DIR/server_conf" "$TEMP_DIR/client_conf";
cp -a "$SERVER_CONF_DIR/"* "$TEMP_DIR/server_conf/" 2>> "$LOG_FILE"; cp -a "$AWG_DIR"/*.conf "$AWG_DIR"/*.png "$AWG_DIR"/setup.conf "$TEMP_DIR/client_conf/" 2>> "$LOG_FILE" || true;
if ! tar -czf "$BACKUP_FILE" -C "$TEMP_DIR" . ; then echo "[$TIMESTAMP] ERROR: Failed to create archive $BACKUP_FILE" >> "$LOG_FILE"; rm -rf "$TEMP_DIR"; exit 1; fi; rm -rf "$TEMP_DIR"; chmod 600 "$BACKUP_FILE" || echo "[$TIMESTAMP] WARN: Failed to chmod backup file." >> "$LOG_FILE";
find "$BACKUP_DIR" -maxdepth 1 -name "awg_backup_*.tar.gz" -printf '%T@ %p\n' | sort -nr | tail -n +$(($MAX_BACKUPS + 1)) | cut -d' ' -f2- | xargs -r rm -f;
echo "[$TIMESTAMP] Backup created: $BACKUP_FILE" >> "$LOG_FILE"; logger -t amneziawg-backup "Backup created: $BACKUP_FILE"; exit 0;
EOF
    chmod +x "$bs" || die "Ошибка chmod $bs"; echo "0 3 * * * root $bs" > /etc/cron.d/backup_amneziawg || log_warn "Ошибка cron"; log "Ежедневный бэкап в $bd настроен."; log "Создание первого бэкапа..."; if "$bs"; then log "OK."; else log_warn "Ошибка."; fi
}
setup_log_rotation() {
    log "Настройка ротации логов..."; cat > /etc/logrotate.d/amneziawg-installer << EOF || log_warn "Ошибка logrotate";
$LOG_FILE
$AWG_DIR/manage_amneziawg.log
$AWG_DIR/backups/backup.log {
    weekly; rotate 12; compress; delaycompress; missingok; notifempty; create 640 root adm; su root adm;
}
EOF
...
create_diagnostic_report() {
    ...
    { echo "=== AMNEZIAWG DIAG REPORT ==="; date; hostname; echo "--- OS ---"; lsb_release -ds; uname -a; echo ""; echo "--- CONFIG ($CONFIG_FILE) ---"; cat "$CONFIG_FILE" 2>/dev/null || echo "N/A"; echo "";
      ...
      echo "--- Listening Ports ---"; ss -lunp; echo ""; echo "--- Firewall Status ---"; if command -v ufw &>/dev/null; then ufw status verbose; else echo "UFW N/A"; fi; echo "";
      echo "--- Routing Table ---"; ip route; echo ""; echo "--- Kernel Params ---"; sysctl net.ipv4.ip_forward net.ipv6.conf.all.disable_ipv6 2>/dev/null; sysctl -a | grep 'rp_filter\|icmp_.*' | grep ipv4 ; echo "";
      echo "--- AWG Journal (last 50) ---"; journalctl -u awg-quick@awg0 -n 50 --no-pager --output=cat; echo ""; echo "--- Client List ---"; grep "^#_Name = " "$SERVER_CONF_FILE" | sed 's/^#_Name = //' || echo "N/A"; echo "";
      echo "--- DKMS Status ---"; dkms status 2>/dev/null || echo "N/A"; echo ""; echo "--- Module Info ---"; modinfo amneziawg 2>/dev/null || echo "N/A"; echo ""; echo "=== END ===";
    } > "$rf" || log_error "Ошибка записи отчета."; chmod 600 "$rf"; log "Отчет: $rf";
}
...
if [ "$VERBOSE" -eq 1 ]; then set -x; fi # Включаем отладку bash, если нужно

initialize_setup # Инициализация для установки
...
while (( current_step < 99 )); do
    ...
    case $current_step in
        ...
        *) die "Ошибка: Неизвестный шаг состояния $current_step.";;
    esac
