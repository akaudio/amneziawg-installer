...
# --- Функции ---
log_msg() { local type="$1"; local msg="<span class="math-inline">2"; local ts; ts\=</span>(date +'%F %T'); local safe_msg; safe_msg=$(echo "$msg" | sed 's/%/%%/g'); local entry="[$ts] $type: $safe_msg"; local color_start=""; local color_end="\033[0m"; if [[ "$NO_COLOR" -eq 0 ]]; then case "<span class="math-inline">type" in INFO\) color\_start\="\\033\[0;32m";; WARN\) color\_start\="\\033\[0;33m";; ERROR\) color\_start\="\\033\[1;31m";; DEBUG\) color\_start\="\\033\[0;36m";; \*\) color\_start\=""; color\_end\="";; esac; fi; if \! mkdir \-p "</span>(dirname "$LOG_FILE")" || ! echo "$entry" >> "$LOG_FILE"; then echo "[$ts] ERROR: Ошибка записи лога $LOG_FILE" >&2; fi; if [[ "$type" == "ERROR" || "<span class="math-inline">type" \=\= "WARN" \]\]; then printf "</span>{color_start}%s${color_end}\n" "$entry" >&2; elif [[ "$type" == "DEBUG" && "<span class="math-inline">VERBOSE" \-eq 1 \]\]; then printf "</span>{color_start}%s${color_end}\n" "$entry" >&2; elif [[ "<span class="math-inline">type" \=\= "INFO" \]\]; then printf "</span>{color_start}%s${color_end}\n" "<span class="math-inline">entry"; else printf "</span>{color_start}%s${color_end}\n" "$entry"; fi; }
...
AWG\_PORT\=</span>{AWG_PORT:-<span class="math-inline">default\_port\}; AWG\_TUNNEL\_SUBNET\=</span>{AWG_TUNNEL_SUBNET:-<span class="math-inline">default\_subnet\};
DISABLE\_IPV6\=</span>{DISABLE_IPV6:-1}; # По умолчанию 1, если в файле нет
        ...
        AWG_PORT=$default_port; AWG_TUNNEL_SUBNET=<span class="math-inline">default\_subnet;
DISABLE\_IPV6\="default"; ALLOWED\_IPS\_MODE\="default"; ALLOWED\_IPS\="";
fi
...
AWG\_PORT\=</span>{CLI_PORT:-<span class="math-inline">AWG\_PORT\}; AWG\_TUNNEL\_SUBNET\=</span>{CLI_SUBNET:-$AWG_TUNNEL_SUBNET};
    ...
    if [[ "<span class="math-inline">config\_exists" \-eq 0 \]\]; then
...
step99_finish() {
    ...
    log "Удаление файла состояния установки..."; rm -f "$STATE_FILE" || log
