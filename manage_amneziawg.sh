...
list_clients() {
    ...
    echo "$clients" | while IFS= read -r name; do
        ...
        local cf="?"; local png="?"; local pk="-"; local ip="-"; local st="Нет данных"; local color_st="\033[0;37m";
            ...
            if [[ -n "$current_pk" ]]; then
                ...
                if echo "$awg_stat" | grep -qF "$current_pk"; then
                     ...
                     if [[ -n "$handshake_line" && ! "$handshake_line" =~ "never" ]]; then
                         if echo "$handshake_line" | grep -q "seconds ago"; then local seconds_ago; seconds_ago=$(echo "$handshake_line" | grep -oP '\d+(?= seconds ago)'); if [[ "$seconds_ago" -lt 180 ]]; then st="Активен"; color_st="\033[1;32m"; ((act++)); else st="Недавно"; color_st="\033[1;33m"; ((act++)); fi; else st="Недавно"; color_st="\033[1;33m"; ((act++)); fi
        ...
        if [ $verbose -eq 1 ]; then printf "%-20s | %-7s | %-7s | %-15s | %-15s | ${color_st}%s${color_end}\n" "$name" "$cf" "$png" "$ip" "$pk" "$st"; else printf "%-20s | %-7s | %-7s | ${color_st}%s${color_end}\n" "$name" "$cf" "$png" "$st"; fi
    done; echo ""; log "Всего клиентов: $tot, Активных/Недавно: $act";
}
usage() {
    exec >&2; echo ""; echo "Скрипт управления AmneziaWG (v2.1)"; echo "=============================================="; echo "Использование: $0 [ОПЦИИ] <КОМАНДА> [АРГУМЕНТЫ]"; echo "";
